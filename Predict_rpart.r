...
#For that, many model systems in R use the same function, conveniently called predict(). 
#Every modeling paradigm in R has a predict function with its own flavor, 
#but in general the basic functionality is the same for all of them.
...
#Always make sure the variable names you use are the same as used in the model. When you do that, you simply call the predict() 
#function with the suited arguments, like this:

predict(Model, newdata=new.cars)
    1    2    3
28.19952 24.45839 18.04503
#So, the lightest car has a predicted mileage of 28.2 miles per gallon and the heaviest car has a predicted mileage of 18 miles per gallon, 
#according to this model. Of course, if you use an inadequate model, your predictions can be pretty much off as well.

#Confidence in your predictions
I#n order to have an idea about the accuracy of the predictions, you can ask for intervals around your prediction.
...
#Now you know that — according to your model — a car with a weight of 2.4 tons has, on average, a mileage between 23 and 25.9 miles per gallon. 
...
#If you’d rather construct your own confidence interval, 
#you can get the standard errors on your predictions as well by setting the argument se.fit to TRUE. 
#You don’t get a vector or a matrix; instead, you get a list with an element fit that contains the 
#predictions and an element se.fit that contains the standard errors.
